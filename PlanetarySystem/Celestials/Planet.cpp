...
namespace Lina{ namespace Planetarium{
    ...
    b8 Planet::spawn(Math::Point3D& pos)
    {
      auto* planet = lnew<ECS::Entity>();
      auto* ico = lnew<Graphics::Shapes::Icosphere>();
      ico->constructSphere(0.5f);
      ...
      auto* transform = lnew(ECS::Component::Transform(pos, rotation));
    ...
    b8 Planet::spawn(nlohmann::json& j)
    {
      auto* planet = lnew<ECS::Entity>();
      auto* ico = lnew<Graphics::Shapes::Icosphere>();
      ico->constructSphere(std::stof(std::string(j["radius"])));
      ico->subdivide(3);

      auto* texture = lnew<Graphics::Texture>();
      texture->setTextureFromPath("../Assets/Universe/" + std::string(j["texture"]));
      ...
      Math::Matrix3D rotation = Math::Util::rotationMatrix(0, Math::Util::zAxis());
        Math::Point3D pos = Math::Point3D(
                std::stof(std::string(j["x"])),
                std::stof(std::string(j["y"])),
                std::stof(std::string(j["z"]))
                );
      auto* transform = lnew(ECS::Component::Transform(pos, rotation));
